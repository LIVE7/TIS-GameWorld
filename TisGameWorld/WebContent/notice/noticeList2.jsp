...
<div class="section">
	<div class="row">
		<div align="center" id="bbs" class="col-md-10 col-md-offset-1">
            ...
            %>
			<p>
				<a href="<%=ctx%>/noticeList.do">글목록</a>
			<p>
			<%
                	}
            %>	<table class="table table-condensed table-striped">
				...
				<c:if test="${boardArr !=null and not empty boardArr}">
					<c:forEach var="notice" items="${boardArr}">
						<tr>
							<td><c:out value="${notice.idx}" /></td>
							<td align="left"><a href="noticeView.do?idx=${notice.idx}">
									<c:out value="${notice.subject}" />
									<c:if test="${notice.readnum>=10}">
									<img src="./images/hot2.gif">
									</c:if>
							</a></td>
							<td><c:out value="${notice.name}" /></td>
							<td><c:out value="${notice.wdate}" /></td>
							<td><c:out value="${notice.readnum}" /></td>
						</tr>
					...
					<c:set var="qstr" value="&findType=${findType}&findKeyword=${findKeyword}"/><!-- -문자열 생성한 변수를 선언하고 싶을때 set  -->
						<ul class="pagination">
							<c:if test="${prevBlock>0}">
								<li><a href="noticeList.do?cpage=${firstCount}${qstr}">맨 앞으로</a></li>
								<li><a href="noticeList.do?cpage=${prevBlock}${qstr}">이전</a></li>
							</c:if>
							...
							class='active'
									...
									href="noticeList.do?cpage=${i}${qstr}">${i}</a></li>
							</c:if>		
									
							</c:forEach>
							<c:if test="${nextBlock<=pageCount}">
								<li><a href="noticeList.do?cpage=${nextBlock}${qstr}">다음</a></li>
								<li><a href="noticeList.do?cpage=${lastCount}${qstr}">맨 뒤로</a></li>
							</c:if>
						</ul>
					</td>
			...
			<div class="section">
				<div class="row">
					<!-- 검색 폼 시작--------------------- -->
					<form name="sf" action="noticeList.do" onsubmit="return check()">
					<!-- hidden data---------------------------------------- -->
					...
					<!-- 페이지 사이즈 폼 시작-------------- -->
					<form name="pf" id="pf" action="noticeList.do">
						<input type="hidden" name="cpage" value="${cpage}">
