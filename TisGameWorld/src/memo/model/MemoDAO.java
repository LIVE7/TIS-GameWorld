...
public class MemoDAO {
	...
	public int insertMemo(MemoVO memo) throws SQLException{ //=insertMemo() 메소드에 MemoVO클래스를 넣고 memo라고 지칭함
		try {
			con=DBUtil.getCon();
			String sql="INSERT INTO tisgame_jellyfish_memo VALUES(tisgame_jellyfish_memo_seq.nextval,?,?,sysdate)";
	...
	public int deleteMemo(int idx) throws SQLException{//-idx가 primary key이기 때문
		try {
			con=DBUtil.getCon();
			//delete문 작성
			String sql="DELETE FROM tisgame_jellyfish_memo WHERE idx=?";
			ps=con.prepareStatement(sql);
			ps.setInt(1,idx);
	...
	public int getTotalCount() throws SQLException {
		try {
			con=DBUtil.getCon();
			String sql="SELECT count(idx) FROM tisgame_jellyfish_memo";//-pk로 넣어야 빠르게 수행가능
			ps=con.prepareStatement(sql);
			rs=ps.executeQuery();
			int total=0;
			if(rs.next()) { //-단일값이기 때문
	...
	public ArrayList<MemoVO> listMemo(int start, int end) throws SQLException{//MemoVO의 데이터를 하나 차곡차곡 쌓아 보이게 하기 위해 arraylist //ArrayList<MemoVO>형을 반환하는 listMemo()메소드
		try {
			...
			String sql="select * from(" + 
					...
					" (select * from tisgame_jellyfish_memo order by idx desc) a" + 
	...
	public MemoVO selectMemoByIdx(int idx) throws SQLException{ //-idx가 primary key이기 때문에 idx로 받아옴
		try {
			con=DBUtil.getCon();
			String sql="SELECT * FROM tisgame_jellyfish_memo WHERE idx=?";
			ps=con.prepareStatement(sql);
			ps.setInt(1, idx);
			rs=ps.executeQuery();
	...
	public int updateMemo(MemoVO memo) throws SQLException{
		try {
			con=DBUtil.getCon();
			String sql="UPDATE tisgame_jellyfish_memo SET name=?, msg=? WHERE idx=?";
			ps=con.prepareStatement(sql);
			ps.setString(1, memo.getName());
			ps.setString(2, memo.getMsg());
	...
	public ArrayList<MemoVO> findMemo(String colType, String keyword) throws SQLException{
		try {
			...
			if(colType.contentEquals("idx")) {
				sql= "SELECT * FROM tisgame_jellyfish_memo WHERE "+colType+" = ?";
				colVal=keyword;
			}else {
				sql= "SELECT * FROM tisgame_jellyfish_memo WHERE "+colType+" LIKE ?";
				colVal="%"+keyword+"%";
			}
			ps=con.prepareStatement(sql);
